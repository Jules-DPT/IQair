...
const scoreCellW = computed(() => pluginWidth.value * 0.40)
...
const tileCellW = computed(() => pluginWidth.value * 0.25)
...
const subIndexTemp  = computed(() =>
  isHorsSaisonChauffe() && thetaRm.value !== null
    ? getSubIndexTempHorsSaisonChauffe(api.tempIndoor ?? 0, thetaRm.value)
    : getSubIndexTemp(api.tempIndoor)
)
const subIndexHum   = computed(() => getSubIndexHum(api.humIndoor))
const subIndexCO2   = computed(() => getSubIndexCO2(api.co2))
const subIndexPM25  = computed(() => getSubIndexPM25(api.pm25Indoor))
const globalScore   = computed(() =>
  Math.min(subIndexTemp.value, subIndexHum.value, subIndexCO2.value, subIndexPM25.value)
)

// ── Canvas wind animation (identical to original) ─────────────────────────
function startWindAnimation() {
  ...
  canvasRo.observe(canvas.parentElement!)
  resizeCanvas()
  ...
  function draw() {
    ...
    for (let i = -60; i < 60; i += 1) {
      ctx.strokeStyle = windColor.value + '80'
      ctx.lineWidth   = 0.8
      ctx.beginPath()
      ctx.moveTo(0, h / 2)
    ...
    ctx.arc(w / 2, h / 2, radius, 0, Math.PI * 2)
    ctx.lineWidth   = 5
    ctx.strokeStyle = windColor.value
    ctx.stroke()
...
onMounted(() => {
  timer = setInterval(() => api.fetchPlugin_meteoIQAir(), 300_000)
...
watch(() => api.tempOutdoor, (newTemp) => {
  ...
  thetaRm.value = thetaRm.value === null
    ? newTemp
    : updateThetaRm(thetaRm.value, newTemp)
})

watch(windCanvas, async (canvas) => {
