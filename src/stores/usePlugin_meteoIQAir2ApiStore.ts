...
export const usePlugin_meteoIQAir2ApiStore = defineStore('plugin-plugin_meteoIQAir2-api', {
  state: () => ({
    ...
    config: {
      ...
      // Notez : VITE_IQAIR_DEVICE_ID2 pour distinguer du premier capteur
  ...
  actions: {
    // Alias uniforme appelé par le widget (identique au store 1)
    async fetchPlugin_meteoIQAir() {
      return this.fetchPlugin_meteoIQAir2()
    },

    async fetchPlugin_meteoIQAir2(type: 'instant' | 'hourly' | 'daily' = 'instant') {
      // Restaurer le cache spécifique au capteur 2
      try {
        const raw = localStorage.getItem('iqair2-cache')
        if (raw) {
      ...
      if (!this.config.userId || !this.config.deviceId || !this.config.token) {
        this.error = 'Configuration manquante (capteur 2) — veuillez renseigner vos identifiants IQAir via le panneau ⚙️'
        return
      }
      ...
      try {
        ...
        // Cache séparé pour ne pas écraser le capteur 1
        localStorage.setItem('iqair2-cache', JSON.stringify({
        ...
        this.error = e.response?.data?.message || e.message || 'Erreur lors de la récupération IQAir (capteur 2)'
        console.error('IQAir2 Store Error:', e)
      } finally {
        this.loading = false
      }
    },
